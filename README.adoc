...
When running `docker compose up`, the main services are available at:

* *Terminus*: `http://localhost:2300` by default, or the port configured by `HANAMI_PORT` in `.env`.
* *n8n*: `http://localhost:5678`.

Run `docker compose ps` to confirm the currently published ports. Device-facing URLs are controlled by `API_URI` for Terminus and `WEBHOOK_URL` for n8n.

=== OVH VPS Deployment

For the personal OVH VPS setup with Terminus, n8n, and HTTPS, see link:doc/ovh_vps_terminus_n8n.md[OVH VPS guide for Terminus and n8n].
